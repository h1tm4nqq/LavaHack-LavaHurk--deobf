...
public enum Class1254
{
    Field13685("StateOfOption", 0), 
    Field13686("ShaderStateOfOption", 1), 
    Field13687("UniformOfShader", 2);
    ...
    private Class1254(final String name, final int ordinal) {
    }
    
    private static String Method5061(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x52B7 ^ 0x38));
            }
