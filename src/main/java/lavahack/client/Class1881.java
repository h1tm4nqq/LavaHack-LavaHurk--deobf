...
public enum Class1881
{
    Field16673("Instant", 0), 
    Field16674("Manual", 1);
    ...
    private Class1881(final String name, final int ordinal) {
    }
    
    private static String Method6976(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x66EE ^ 0xE));
            }
