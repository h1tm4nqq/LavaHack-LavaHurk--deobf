...
public enum Class1480
{
    Field14901("Time", 0), 
    Field14902("Distance", 1);
    ...
    private Class1480(final String name, final int ordinal) {
    }
    
    private static String Method5922(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x4665 ^ 0x26));
            }
