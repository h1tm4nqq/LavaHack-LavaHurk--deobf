...
public enum Class1745
{
    ...
    private Class1745(final String name, final int ordinal) {
    }
    
    private static String Method6666(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x6714 ^ 0x5A));
            }
