...
public enum Class652
{
    Field10794("Vertical", 0), 
    Field10795("Square", 1);
    ...
    private Class652(final String name, final int ordinal) {
    }
    
    private static String Method2821(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x50B0 ^ 0x79));
            }
