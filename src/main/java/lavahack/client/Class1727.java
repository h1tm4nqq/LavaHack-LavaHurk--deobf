...
public enum Class1727
{
    Field15929("Distance", 0), 
    Field15930("Health", 1), 
    Field15931("Damage", 2);
    ...
    private Class1727(final String name, final int ordinal) {
    }
    
    private static String Method6598(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x31A6 ^ 0x85));
            }
