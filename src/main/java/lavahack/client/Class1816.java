...
enum Class1816
{
    Field16346("PacketMineProvider", 0), 
    Field16347("PlayerDamageBlock", 1), 
    Field16348("LeftClick", 2);
    ...
    private Class1816(final String name, final int ordinal) {
    }
    
    private static String Method6841(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0xADC ^ 0xA));
            }
