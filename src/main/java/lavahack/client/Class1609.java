...
public enum Class1609
{
    Field15509("NOT_YET_CONNECTED", 0), 
    Field15510("OPEN", 1), 
    Field15511("CLOSING", 2), 
    Field15512("CLOSED", 3);
    ...
    private Class1609(final String name, final int ordinal) {
    }
    
    private static String Method6277(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x2C5D ^ 0x55));
            }
