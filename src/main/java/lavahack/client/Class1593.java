...
public enum Class1593
{
    ...
    private Class1593(final String name, final int ordinal) {
    }
    
    private static String Method6253(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x3425 ^ 0x7E));
            }
