...
public enum Class1597
{
    Field15474("MATCHED", 0), 
    Field15475("NOT_MATCHED", 1);
    ...
    private Class1597(final String name, final int ordinal) {
    }
    
    private static String Method6255(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x3022 ^ 0xBD));
            }
