...
enum Class849
{
    ...
    private Class849(final String name, final int ordinal) {
    }
    
    private static String Method3586(final String s) {
        if (s != null) {
            final char[] charArray = s.toCharArray();
            final char[] value = new char[charArray.length];
            for (int i = 0; i < charArray.length; ++i) {
                value[i] = (char)(charArray[i] ^ (0x69EB ^ 0x4));
            }
